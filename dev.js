...
const REPO_NAME = "fame-frames-1524";
...
`).join('\n\n');
        ...
        const fullPrompt = `
You are an expert AI web developer acting as a GitHub agent with full access to this repository. Your task is to intelligently modify the website based on the user's request.

**CRITICAL RULES:**
1.  **INTEGRATE NEW PAGES:** If you create a new HTML file (e.g., 'about.html'), you MUST add a navigation link to it in the other existing HTML file(s) (e.g., in a `<nav>` section of 'index.html'). The user must be able to navigate between all pages.
2.  **ANALYZE & SELF-CORRECT:** Before finalizing your response, analyze your proposed changes.
    *   Did you add a new page? If so, did you link to it from the navigation menu on other pages?
    *   Are all file paths in links (`<a>`) and assets (`<img>`) correct?
    *   Is the code complete and free of placeholder comments?
    *   If you find an error in your plan, fix it before responding.
3.  **BE EFFICIENT:** Only return files that have been changed or are newly created. Do not return files that are not changed. The JSON output must only contain the files you are adding or modifying.
...
Based on all the rules and the user's request, provide the necessary file creations or updates.
`;
